    ...
    <header className="shrink-0 flex items-center gap-3 px-4 pt-3 pb-2">
      <div className="flex items-center gap-2 min-w-0 flex-1">
        ...
        <div className="min-w-0 flex flex-col">
          <h2 className="font-display text-base font-bold tracking-widest text-[rgb(200,170,110)] uppercase truncate">
            {title}
          </h2>
          {subtitle ? (
            <span className="text-[10px] font-bold uppercase tracking-wider text-[rgba(200,170,110,0.6)] truncate">
              {subtitle}
            </span>
          ) : null}
          {status !== 'connected' ? (
      ...
      <div className="flex items-center gap-2 shrink-0">
        {badges?.map((badge) => (
          <span
            key={badge.label}
            className="flex items-center gap-1 rounded-full border border-[rgba(200,170,110,0.4)] bg-[rgba(10,20,40,0.6)] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[rgba(200,170,110,0.8)]"
          >
            {badge.icon}
            {badge.label}
          </span>
