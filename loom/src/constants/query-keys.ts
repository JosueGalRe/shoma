...
export const queryKeys = {
  championDetail: (championId: string | number) => {
    return [DDRAGON_QUERY_KEY, CHAMPION_DETAIL_QUERY_KEY, championId] as const
  },
  championSkins: (championId: string | number) => {
    return [DDRAGON_QUERY_KEY, CHAMPION_SKINS_QUERY_KEY, championId] as const
  },
  champions: () => {
    return [DDRAGON_QUERY_KEY, CHAMPIONS_QUERY_KEY] as const
  },
  currentSummoner: () => {
    return [LCU_QUERY_KEY, 'summoner', CURRENT_QUERY_KEY] as const
  },
  ddragon: () => {
    return [DDRAGON_QUERY_KEY] as const
  },
  gameflow: () => {
    return [GAMEFLOW_QUERY_KEY] as const
  },
  groups: () => {
    return [LCU_QUERY_KEY, GROUPS_QUERY_KEY] as const
  },
  invites: () => {
    return [INVITES_QUERY_KEY] as const
  },
  latestVersion: () => {
    return [DDRAGON_QUERY_KEY, LATEST_VERSION_QUERY_KEY] as const
  },
  lcu: () => {
    return [LCU_QUERY_KEY] as const
  },
  lobby: () => {
    return [LOBBY_QUERY_KEY] as const
  },
  profileIcon: () => {
    return [DDRAGON_QUERY_KEY, PROFILE_ICON_QUERY_KEY] as const
  },
  queue: () => {
    return [QUEUE_QUERY_KEY] as const
  },
  queueSearch: () => {
    return [QUEUE_SEARCH_QUERY_KEY] as const
  },
  readyCheck: () => {
    return [READY_CHECK_QUERY_KEY] as const
  },
  runes: () => {
    return [DDRAGON_QUERY_KEY, RUNES_QUERY_KEY] as const
  },
  searchState: () => {
    return [LCU_QUERY_KEY, SEARCH_STATE_QUERY_KEY] as const
  },
  sentInvites: () => {
    return [SENT_INVITES_QUERY_KEY] as const
  },
  session: () => {
    return [LCU_QUERY_KEY, 'lobby', SESSION_QUERY_KEY] as const
  },
  summoner: (summonerId: string | number) => {
    return [LCU_QUERY_KEY, SUMMONERS_QUERY_KEY, summonerId] as const
  },
} as const
