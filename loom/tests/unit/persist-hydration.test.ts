...
async function loadSettingsStore(): Promise<SettingsModule> {
  ...
  return import(`../../src/core/state/settings-store?${cacheKey}`) as Promise<SettingsModule>
}

async function loadSessionStore(): Promise<SessionModule> {
  const cacheKey = `test=${Date.now()}-${Math.random()}`
  return import(`../../src/core/state/session-store?${cacheKey}`) as Promise<SessionModule>
}
