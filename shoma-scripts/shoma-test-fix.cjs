...
;(async () => {
  ...
  const memberName = await page.evaluate(() => {
    const memberElements = Array.from(document.querySelectorAll('*')).filter((el) => el.textContent?.includes('MIDDLE / FILL'))
  ...
  await page.screenshot({ path: '/tmp/shoma-test-fix.png', fullPage: true })
