...
export class RiftClient {
  ...
  async send(payload: string) {
    ...
    const encryptedBuffer = await window.crypto.subtle.encrypt({
      name: "AES-CBC",
      iv
    }, this.#sharedKey, utf8ToBuffer(payload));
  ...
  async #sendIdentity(publicKey: string) {
    ...
    this.#sharedKey = await window.crypto.subtle.importKey("raw", secret.buffer, {
      name: "AES-CBC"
  ...
  async #handleRelayPayload(payload: unknown) {
    if (this.#encrypted && this.#sharedKey && typeof payload === "string") {
      ...
      }, this.#sharedKey, base64ToBuffer(parsed.encrypted));
