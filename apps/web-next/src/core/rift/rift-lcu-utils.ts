...
export function frameIsUpdate(frame: [number, ...unknown[]]): frame is [typeof MobileOpcode.UPDATE, string, number, unknown] {
  return frame[0] === MobileOpcode.UPDATE
    && typeof frame[1] === "string"
    && typeof frame[2] === "number";
}

export function frameIsResponse(frame: [number, ...unknown[]]): frame is [typeof MobileOpcode.RESPONSE, number, number, unknown] {
  return frame[0] === MobileOpcode.RESPONSE
    && typeof frame[1] === "number"
    && typeof frame[2] === "number";
}

export function frameIsVersionResponse(frame: [number, ...unknown[]]): frame is [typeof MobileOpcode.VERSION_RESPONSE, string, string] {
  return frame[0] === MobileOpcode.VERSION_RESPONSE
    && typeof frame[1] === "string"
