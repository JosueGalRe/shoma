    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col justify-center px-5 py-10 sm:px-8">
        <Card className="rounded-3xl border-white/60 bg-white/75 p-6 shadow-2xl backdrop-blur sm:p-10">
          <h1 className="font-display text-3xl text-ink">{t($ => $.connected.unavailableTitle)}</h1>
          <p className="mt-3 text-slate-700">{t($ => $.connected.unavailableBody)}</p>
          <Button asChild className="mt-6 h-12 rounded-2xl bg-ink px-5 font-display text-mist hover:bg-slate">
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col justify-center px-5 py-10 sm:px-8">
      <Card className="rounded-3xl border-white/60 bg-white/75 p-6 shadow-2xl backdrop-blur sm:p-10">
        <div className="flex items-center justify-between gap-3">
          <h1 className="font-display text-4xl leading-tight text-ink">{t($ => $.connected.title)}</h1>
          <div className="flex items-center gap-2">
            <LanguageSwitcher />
            <Button asChild variant="outline" className="h-11 rounded-2xl border-slate-300 px-4 font-display text-slate-700">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          <Card className="rounded-2xl border-slate-200 bg-white p-4">
            <h3 className="font-display text-sm uppercase tracking-[0.2em] text-slate">{t($ => $.connected.desktop)}</h3>
          ...
          <Card className="rounded-2xl border-slate-200 bg-white p-4">
            <h3 className="font-display text-sm uppercase tracking-[0.2em] text-slate">{t($ => $.connected.queue)}</h3>
            {queueState ? (
          ...
          <Card className="rounded-2xl border-slate-200 bg-white p-4 sm:col-span-2">
            <h3 className="font-display text-sm uppercase tracking-[0.2em] text-slate">{t($ => $.connected.lobby)}</h3>
            {lobbyDetails ? (
              <div className="mt-2 grid gap-2 text-slate-700 sm:grid-cols-2">
                ...
                <p>
                  <Trans
                    ...
                    values={{ value: lobbyDetails.queueName ?? lobbyDetails.queueId ?? t($ => $.connected.unknown) }}
                  />
                </p>
                <p>
                  <Trans
                    ...
                    values={{ value: lobbyDetails.mapName ?? lobbyDetails.mapId ?? t($ => $.connected.unknown) }}
                  />
                </p>
              </div>
            ) : (
        ...
        {logLines.length > 0 ? (
          <Card className="mt-8 rounded-2xl border-slate-200 bg-white p-4">
            <h3 className="font-display text-sm uppercase tracking-[0.2em] text-slate">{t($ => $.connected.relayPreview)}</h3>
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              {logLines.map((line) => (
                <li className="rounded-lg bg-slate-50 px-3 py-2" key={line}>{line}</li>
              ))}
            </ul>
          </Card>
        ) : null}
