...
export function ConnectEntryForm({
  code,
  codeError,
  register,
  handleSubmit,
  ...
  return (
    <form className="mt-8 flex flex-col gap-3 sm:flex-row" onSubmit={handleSubmit(onSubmit)}>
      <div className="w-full">
        <Input
          className="h-14 w-full rounded-2xl border border-slate-200 bg-white px-4 text-center font-display text-2xl tracking-[0.35em] outline-none transition focus:border-brass focus:ring-2 focus:ring-brass/30"
          inputMode="numeric"
          maxLength={6}
        ...
        />
        {codeError ? (
          <p className="mt-1 text-sm text-red-700">{codeError.message}</p>
        ) : null}
      </div>
      <Button
        className="h-14 shrink-0 rounded-2xl bg-ink px-6 font-display text-lg text-mist transition hover:bg-slate disabled:cursor-not-allowed disabled:bg-slate-400"
