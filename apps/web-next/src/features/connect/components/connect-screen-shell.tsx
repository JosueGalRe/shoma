    ...
    <main className='mx-auto flex min-h-screen w-full max-w-3xl flex-col justify-center px-5 py-10 sm:px-8'>
      <section className='rounded-3xl border border-white/60 bg-white/75 p-6 shadow-2xl backdrop-blur sm:p-10'>
        <div className='flex items-start justify-between gap-4'>
          <p className='font-display text-slate text-sm tracking-[0.24em] uppercase'>{t(($) => $.connect.brand)}</p>
          <LanguageSwitcher />
        </div>
        <h1 className='font-display text-ink mt-3 text-4xl leading-tight sm:text-5xl'>{t(($) => $.connect.heading)}</h1>
        <p className='mt-4 text-base text-slate-700'>{t(($) => $.connect.subtitle)}</p>
        ...
        {status === RiftClientState.CONNECTED ? (
          <Card className='mt-8 rounded-2xl border-slate-200 bg-white p-4 text-slate-700'>
            ...
            <Button asChild className='bg-ink font-display text-mist hover:bg-slate mt-4 h-11 rounded-2xl px-5'>
