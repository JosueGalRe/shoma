...
export function useRiftLcuRuntime({ client, status, setPeer, appendLog }: UseRiftLcuRuntimeOptions) {
  ...
  const lcuTransport = useMemo(() => new RiftLcuTransport({
    async send(payload) {
  ...
  const lcuQueries = useMemo(() => createLcuQueryFactory((path, method = "GET", body) => {
    return lcuTransport.request(path, method, body);
  }), [lcuTransport]);

  async function getQueueDescription(queueId: number): Promise<string | null> {
    const queueInfo = await queryClient.ensureQueryData(
      lcuQueries.queueInfo.options(queueId)
    );
  ...
  async function getMapName(mapId: number): Promise<string | null> {
    const mapInfo = await queryClient.ensureQueryData(
      lcuQueries.mapInfo.options(mapId)
    );
