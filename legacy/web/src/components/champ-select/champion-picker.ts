...
export default class ChampionPicker extends Vue {
  ...
  created() {
    ...
    this.$root.observe('/lol-champ-select/v1/pickable-champion-ids', (result) => {
      this.pickableChampions = (result.status === 200 ? result.content : this.pickableChampions).filter(
        (x: number) => !!this.$parent.championDetails[x],
      )
      this.pickableChampions.sort((a, b) =>
        this.$parent.championDetails[a].name.localeCompare(this.$parent.championDetails[b].name),
      )
    })

    this.$root.observe('/lol-champ-select/v1/bannable-champion-ids', (result) => {
      this.bannableChampions = (result.status === 200 ? result.content : this.bannableChampions).filter(
        (x: number) => !!this.$parent.championDetails[x],
      )
      this.bannableChampions.sort((a, b) =>
        this.$parent.championDetails[a].name.localeCompare(this.$parent.championDetails[b].name),
      )
    })
  }
  ...
  get selectableChampions(): number[] {
    ...
    const isCurrentlyBanning =
      ...
      this.$parent.currentTurn.filter((x) => x.type === 'ban' && x.actorCellId === this.state.localPlayerCellId && !x.completed)
        .length > 0
  ...
  hoverChampion(championId: number) {
    ...
    this.$root.request(
      '/lol-champ-select/v1/session/actions/' + firstUncompletedPick.id,
      'PATCH',
      JSON.stringify({ championId }),
    )
  }
  ...
  completeAction() {
    const act = this.$parent.getActions(this.state.localPlayer)!
    this.$root.request(
      '/lol-champ-select/v1/session/actions/' + act.id,
      'PATCH',
      JSON.stringify({
