    ...
    <div class="rune-editor" v-if="show && runes">
      ...
      <div class="page-options">
        <select class="league" @change="$parent.selectRunePage($event)">
          <option :value="rune.id" :selected="rune.isActive" v-for="rune in $parent.runePages.filter((x) => x.isEditable)">
            {{ rune.name }}
          </option>
        </select>
      ...
      <div class="content" v-if="currentPage">
        ...
        <div class="tree-selector">
          <div
            ...
            :class="currentPage.primaryStyleId === tree.id && 'selected'"
            :style="getRuneIconStyle(tree)"
          ></div>
        </div>

        <div class="tree-runes">
          <div
            v-for="(slot, idx) in getRuneTree(currentPage.primaryStyleId).slots"
            class="slot"
            :class="idx === 0 && 'keystone'"
          >
            <div
              v-for="rune in slot.runes"
              :class="currentPage.selectedPerkIds[idx] === rune.id && 'selected'"
              @click="selectPrimaryRune(idx, rune.id)"
              class="rune"
              :style="getRuneIconStyle(rune)"
            ></div>
          </div>
        </div>
        ...
        <div class="tree-selector">
          <div
            ...
            :class="currentPage.subStyleId === tree.id && 'selected'"
            :style="getRuneIconStyle(tree)"
          ></div>
        </div>
        ...
        <div class="tree-runes">
          <div v-for="(slot, idx) in (getRuneTree(currentPage.subStyleId) || { slots: [] }).slots.slice(1)" class="slot">
            <div
              v-for="rune in slot.runes"
              :class="(currentPage.selectedPerkIds[4] === rune.id || currentPage.selectedPerkIds[5] === rune.id) && 'selected'"
