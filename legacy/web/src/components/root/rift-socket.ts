...
export default class RiftSocket {
  ...
  public async send(contents: string) {
    ...
    this.socket.send(JSON.stringify([RiftOpcode.SEND, bufferToBase64(iv.buffer) + ':' + bufferToBase64(encryptedBuffer)]))
  }
  ...
  private async sendIdentity(pubkey: string) {
    ...
    this.key = await (window.crypto.subtle || window.crypto.webkitSubtle).importKey(
      'raw',
      secret.buffer,
      <any>{
        name: 'AES-CBC',
    ...
    const NodeRSA: typeof NodeRSAType = <any>(
      await import(/* webpackPrefetch: true */ /* webpackChunkName: "node-rsa" */ 'node-rsa').then((x) => x.default)
    )
    ...
    this.socket.send(JSON.stringify([RiftOpcode.SEND, [MobileOpcode.SECRET, rsa.encrypt(identify, 'base64', 'utf8')]]))
  }
