...
export const registerProgram = Effect.fn("Relay.register")((body: unknown) =>
  Effect.gen(function* registerProgram() {
    ...
    const validated = yield* Match.value(pubkey).pipe(
      Match.when(
        (p): p is MissingPublicKeyError => p instanceof MissingPublicKeyError,
        (err) => Effect.fail(err),
      ),
...
export const checkProgram = Effect.fn("Relay.check")((query: unknown) =>
  Effect.gen(function* checkProgram() {
    ...
    const validated = yield* Match.value(token).pipe(
      Match.when(
        (t): t is MissingTokenToCheckError => t instanceof MissingTokenToCheckError,
        (err) => Effect.fail(err),
      ),
...
export function setupHttpRoutes(
  ...
  app.ws("/conduit", {
    close(ws) {
      void runRealtime(
        withRealtimeService(getRealtime(), (realtime) => realtime.handleConduitClose(ws)),
      );
    },
    message(ws, message) {
      void runRealtime(
  ...
  app.ws("/mobile", {
    close(ws) {
      void runRealtime(
        withRealtimeService(getRealtime(), (realtime) => realtime.handleMobileClose(ws)),
      );
    },
    message(ws, message) {
      void runRealtime(
        withRealtimeService(getRealtime(), (realtime) => realtime.handleMobileMessage(ws, message)),
      );
    },
    open(ws) {
      void runRealtime(
        withRealtimeService(getRealtime(), (realtime) => realtime.handleMobileOpen(ws)),
      );
    },
  });
}
