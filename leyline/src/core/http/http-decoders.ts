...
export function readPubkeyFromBody(value: unknown): string | null {
  ...
  return Match.value(result).pipe(
    Match.when(
      (r): r is MissingPublicKeyError => r instanceof MissingPublicKeyError,
      () => null,
    ),
...
export function readTokenCode(value: unknown): string | null {
  ...
  return Match.value(result).pipe(
    Match.when(
      (r): r is TokenMissingCodeError => r instanceof TokenMissingCodeError,
      () => null,
    ),
    Match.orElse((r: string) => r),
  );
...
export function extractConduitAuth(data: ConduitOpenData): { token?: string; publicKey?: string } {
  ...
  const { token: queryToken, publicKey: queryPublicKey, publickey: queryPublickey } = query;
  ...
  return Match.value(result).pipe(
    Match.when(
      (r): r is MissingConduitAuthError => r instanceof MissingConduitAuthError,
      () => ({ publicKey, token }),
    ),
    Match.orElse((r) => r),
  );
