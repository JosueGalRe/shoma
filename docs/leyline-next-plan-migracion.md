# Plan de Migración por Fases: leyline

Este plan propone una migración incremental, reversible y orientada a preservar el comportamiento observable de `leyline`. No asume que toda la migración sea obligatoria: cada fase debe aportar valor verificable por sí sola y puede detenerse si el riesgo supera el beneficio.
...
1. Agregar tests directos para `src/core/realtime/realtime-utils.ts`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Tests de realtime-utils pasan
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/realtime-utils.test.ts 2>&1
     ...
     Scenario: No se modifica comportamiento observable de frames
       Tool: Bash
       Steps: cd leyline && bun test tests/integration/websocket-integration.test.ts 2>&1
       Expected: output contiene "pass" y 0 "fail"
...
2. Agregar tests directos para `src/core/realtime/realtime-schemas.ts`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Tests de realtime-schemas pasan
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/realtime-schemas.test.ts 2>&1
       Expected: output contiene "pass" y 0 "fail"
       Evidence: .sisyphus/evidence/fase1-t2-qa.txt

     Scenario: No hay cambios en formato aceptado de frames
       Tool: git diff -- leyline/src/core/realtime/realtime-schemas.ts
...
3. Agregar tests directos para `src/core/http/http-schemas.ts`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Tests de http-schemas pasan
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/http-schemas.test.ts 2>&1
       Expected: output contiene "pass" y 0 "fail"
       Evidence: .sisyphus/evidence/fase1-t3-qa.txt

     Scenario: No se introducen refinamientos semánticos
       Tool: grep
       Steps: pattern='Schema\.refine\|Schema\.length\|Schema\.pattern' path='leyline/src/core/http/http-schemas.ts'
...
4. Agregar tests de caracterización para el límite `runRealtime`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Tests de caracterización de runRealtime pasan
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/runRealtime.test.ts 2>&1
       Expected: output contiene "pass" y 0 "fail"
...
5. Agregar tests para `DatabaseLive` que demuestren el estado actual no inicializado.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Test documenta estado no inicializado
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/database-live.test.ts 2>&1
     ...
     Scenario: Test de regresión de DB pasa
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/realtime.test.ts 2>&1
       Expected: output contiene "pass" y 0 "fail"
...
1. Reemplazar el cast `as Effect.Effect<A, E>` en `serviceEffect`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Cast eliminado del código
       ...
       Steps: pattern='as Effect.Effect<A, E>' paths=['leyline/src/core/realtime/realtime-service.ts']
       Expected: count == 0
       Evidence: .sisyphus/evidence/fase2-t1-qa-cast.txt

     Scenario: Sin errores de tipo tras el cambio
       Tool: lsp_diagnostics
       Steps: filePath='leyline/src/core/realtime/realtime-service.ts' severity='error'
       Expected: count == 0
...
2. Cambiar `runRealtime` para usar `Effect.runPromiseExit`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: runRealtime usa runPromiseExit
       Tool: ast_grep_search
       Steps: pattern='runPromise\(' paths=['leyline/src/index.ts'] context=2
     ...
     Scenario: No hay unhandled rejections en logs de test
       Tool: Bash
       Steps: cd leyline && bun test 2>&1 | grep -i "unhandled\|rejection"
       Expected: count == 0
...
3. Corregir `DatabaseLive` para ejecutar `initialize` durante adquisición.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Layer ejecuta initialize en adquisición
       Tool: grep
       Steps: pattern='Effect.acquireRelease.*makeDatabaseService' path='leyline/src/core/database/database-service.ts' output_mode='content'
     ...
     Scenario: Tests de integración pasan sin duplicar init
       Tool: Bash
       Steps: cd leyline && bun test tests/integration/runtime.test.ts 2>&1
       Expected: output contiene "pass" y 0 "fail"
...
4. Revisar `src/core/effect/runtime.ts`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Código muerto eliminado o documentado
       Tool: grep
       Steps: pattern='makeRuntime' path='leyline/src' output_mode='files_with_matches'
     ...
     Scenario: No hay referencias a runtime.ts desde otros módulos
       Tool: grep
       Steps: pattern='from.*core/effect' path='leyline/src' output_mode='files_with_matches'
     ...
     Scenario: Build pasa tras eliminación
       Tool: Bash
       Steps: cd leyline && bun run build 2>&1 | tail -5
...
5. Ejecutar verificación completa de leyline.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Sin errores de TypeScript
       Tool: lsp_diagnostics
       Steps: filePath='leyline/src' severity='error'
       Expected: count == 0
...
1. Introducir un runtime/layer central para `LoggerService`, `DatabaseService` y `RealtimeStateService`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Dependencias aparecen en tipos de servicios
       Tool: ast_grep_search
       Steps: pattern='RealtimeStateService|DatabaseService|LoggerService' paths=['leyline/src/index.ts'] output_mode='content'
...
2. Reutilizar `LoggerLive` en vez de reconstruir manualmente un logger equivalente en `index.ts`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: LoggerLive se usa en index.ts
       Tool: grep
       Steps: pattern='LoggerLive|Layer\.provide' path='leyline/src/index.ts' output_mode='content'
     ...
     Scenario: Logs mantienen misma estructura
       Tool: Bash
       Steps: cd leyline && bun test tests/integration/runtime.test.ts 2>&1 | grep -i "error\|warn\|info\|debug"
...
3. Migrar gradualmente construcción de realtime hacia `RealtimeLive` o un Layer equivalente.
   ...
   - **QA Scenarios**:
     ...
     Scenario: RealtimeStateService se provee explícitamente
       Tool: grep
       Steps: pattern='Effect\.provideService.*RealtimeStateService|RealtimeLive|Layer\.provide' path='leyline/src/index.ts' output_mode='content'
...
4. Mantener el bridge síncrono de database solo como compatibilidad temporal.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Bridge documentado como legacy
       Tool: grep
       Steps: pattern='@deprecated|legacy bridge|TODO.*remove bridge' path='leyline/src/core/database/database.ts' output_mode='content'
     ...
     Scenario: No hay nuevos consumidores del bridge
       Tool: grep
       Steps: pattern='from.*database\.ts|generateCode|lookup|potentiallyUpdate' path='leyline/src' output_mode='files_with_matches'
     ...
     Scenario: Tests pasan con bridge aislado
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/http-smoke.test.ts 2>&1
       Expected: output contiene "pass" y 0 "fail"
...
5. Agregar tests de integración para startup/shutdown con el runtime centralizado.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Idempotencia de stop verificada
       Tool: Bash
       Steps: cd leyline && bun test tests/integration/runtime.test.ts -t "stop" 2>&1
     ...
     Scenario: Sin sockets activos tras shutdown
       Tool: Bash
       Steps: cd leyline && bun test tests/integration/websocket-integration.test.ts -t "disconnect" 2>&1
       Expected: output contiene "pass" y 0 "fail"
...
1. Migrar errores manuales seleccionados a `Data.TaggedError`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: TaggedError usado en lugar de clases manuales
       Tool: ast_grep_search
       Steps: pattern='Data\.TaggedError' paths=['leyline/src'] output_mode='files_with_matches'
       Expected: count >= 1
     ...
     Scenario: Error-channel tests pasan
       Tool: Bash
       Steps: cd leyline && bun test tests/unit/realtime.test.ts -t "error" 2>&1
       Expected: output contiene "pass" y 0 "fail"
...
2. Consolidar `FrameFormatError` y `FramePayloadError`.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Sin taxonomía duplicada
       Tool: grep
       Steps: pattern='FrameFormatError|FramePayloadError' path='leyline/src' output_mode='files_with_matches'
     ...
     Scenario: Logs externos no cambian
       Tool: Bash
       Steps: cd leyline && bun test tests/integration/websocket-integration.test.ts 2>&1 | grep -i "frame\|format\|payload"
...
3. Eliminar o documentar código muerto confirmado.
   ...
   - **QA Scenarios**:
     ...
     Scenario: No hay exports infrautilizados
       Tool: bash
       Steps: cd leyline && grep -r 'from.*core/effect' src/ --include='*.ts' | grep -v 'index.ts'
       Expected: count == 0
       Evidence: .sisyphus/evidence/fase4-t3-qa-exports.txt

     Scenario: Build y tests pasan tras limpieza
       Tool: Bash
       Steps: cd leyline && bun run build && bun test 2>&1
...
4. Reducir dualidad de logging.
   ...
   - **QA Scenarios**:
     ...
     Scenario: API primaria de logging es Effect service
       Tool: grep
       Steps: pattern='LoggerService' path='leyline/src/index.ts' output_mode='content'
     ...
     Scenario: Fachada síncrona aislada
       Tool: grep
       Steps: pattern='export const logger|syncLogger' path='leyline/src/core/logger/logger-utils.ts' output_mode='content'
     ...
     Scenario: Logs siguen apareciendo en tests
       Tool: Bash
       Steps: cd leyline && bun test tests/integration/runtime.test.ts 2>&1 | grep -c 'info\|warn\|error\|debug'
...
5. Evaluar hardening de seguridad como trabajo separado.
   ...
   - **QA Scenarios**:
     ...
     Scenario: Documento de seguridad existe
       Tool: Bash
       Steps: ls docs/leyline-security-hardening.md 2>/dev/null || ls docs/leyline-plan-migracion.md
       Expected: docs/leyline-security-hardening.md existe O el plan de migración contiene sección explícita de seguridad
       Evidence: .sisyphus/evidence/fase4-t5-qa-doc.txt

     Scenario: No hay cambios de CORS en este plan
       Tool: grep
       Steps: pattern='Access-Control-Allow-Origin|CORS' path='leyline/src/index.ts' output_mode='content'
     ...
     Scenario: Validación semántica no se endurece en esta migración
       Tool: grep
       Steps: pattern='Schema\.length\|Schema\.refine\|Schema\.pattern' path='leyline/src/core/http/http-schemas.ts' output_mode='content'
       Expected: count == 0 (sin refinamientos nuevos)
...
1. Ejecutar `lsp_diagnostics leyline/src`.
2. Ejecutar `bun test` desde `leyline`.
...
```bash
grep -c '#### ' docs/leyline-plan-migracion.md
```
